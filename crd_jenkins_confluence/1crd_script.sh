...
for cr_uid in "${!deployment_files_uids[@]}"; do
    ...
    do
        ...
        if [ "$grafana_uid" == "$cr_uid" ] ; then
                ...
                GrafanaHTML+=$(echo "<p style=\"color:blue;font-size:120%;\"> Dashboard with UID: $cr_uid and version: $Version in Grafana - filename: ${deployment_files_uids[$cr_uid]}- found  </p>")
    ...
    if [ $match_bool_grafana -eq "0" ] ; then 
            ...
            GrafanaHTML+=$(echo "<p style=\"color:red;font-size:120%;\"> Dashboard with UID: $cr_uid and version: $Version in Grafana - filename:${deployment_files_uids[$cr_uid]} -not found  </p>")
...
for cr_uid in "${!deployment_files_uids[@]}"; do
    ...
    for j in "${!k8s_uids_array[@]}"; do
        ...
        # The followong condition will check if there is a version match as well
            if [ "${k8s_ver_array[$j]}" == "$Version" ] ; then 
                ...
                KubeHTML+=$(echo "<p style=\"color:blue;font-size:120%;\"> dashboard with UID: $cr_uid and version: $Version in kubernetes - filename:${deployment_files_uids[$cr_uid]} - found </p>")
                echo "<tr>
                <td>$cr_uid</td>
    ...
    if [ $match_bool_uid -eq "1"  ] && [ $match_bool_version -eq "0" ]; then 
            ...
            KubeHTML+=$(echo "<p style=\"color:red;font-size:120%;\"> dashboard with UID: ${cr_uid} and version: ${Version} in kubernetes - ${deployment_files_uids[$cr_uid]} - Not found  </p>")
            echo "mismatch"
            echo "<tr>
                <td>$cr_uid</td>
                <td>${deployment_files_uids[$cr_uid]}</td>
